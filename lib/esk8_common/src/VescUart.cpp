...
bool ProcessReadPacket(uint8_t* message, struct bldcMeasure& values, int len) {
	...
	case COMM_GET_VALUES:
		...
		values.tempFetFiltered2		= buffer_get_float16(message, 1e1, &ind);
		values.tempMotorFiltered	= buffer_get_float16(message, 1e1, &ind);
		values.tempMotorFiltered2	= buffer_get_float16(message, 1e1, &ind);
		values.avgMotorCurrent		= buffer_get_float32(message, 1e2, &ind);
		values.avgMotorCurrent2		= buffer_get_float32(message, 1e2, &ind);
		values.avgInputCurrent		= buffer_get_float32(message, 1e2, &ind);
		values.avgId				= buffer_get_float32(message, 1e2, &ind);
		values.avgId2				= buffer_get_float32(message, 1e2, &ind);
		values.avgIq				= buffer_get_float32(message, 1e2, &ind);
	  	values.avgIq2				= buffer_get_float32(message, 1e2, &ind);
		values.dutyNow				= buffer_get_float16(message, 1e3, &ind);
		values.dutyNow2				= buffer_get_float16(message, 1e3, &ind);
		values.rpm					= buffer_get_float32(message, 1e0, &ind);
		values.rpm2					= buffer_get_float32(message, 1e0, &ind);
		...
		values.tachometer2			= buffer_get_int32(message, &ind);
		values.tachometerAbs		= buffer_get_int32(message, &ind);
		values.tachometerAbs2		= buffer_get_int32(message, &ind);
...
void VescUartSetCurrent(float current){
	VescUartSetCurrent(current, vesc_io);
}
